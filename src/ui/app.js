...
import { confirmWorldRule, deleteWorldRule, editWorldRule, rejectWorldRule } from '../core/world-rules.js';
import { containerText, escapeHtml as escapePresentationHtml, formatReviewItem, homeModel, ownershipText, pendingOptions, pendingQuestion, quantityText, reviewSummary, shortEvidence } from './presentation.js';
import {
  buildRollingContext,
  canResumeHistoryImport,
  createHistoryFingerprint,
  prepareHistoryChunks,
} from '../core/history-consolidation.js';
import {
    ...
    >
      <header class="fushenglu-header">
        <div>
          ...
          <h1 id="fushenglu-title">浮生錄 <span class="fushenglu-version">· v${APP_VERSION}</span></h1>
          <p class="fushenglu-management-only fushenglu-version-diagnostics">
            manifest version: ${MANIFEST_VERSION} · runtime version: ${RUNTIME_VERSION}
            ${MANIFEST_VERSION !== RUNTIME_VERSION ? '<strong>版本載入異常，可能仍在使用舊快取</strong>' : ''}
          </p>
        </div>
...
export function mountFushengluApp({
  ...
  function renderApi() {
    ...
    screen.innerHTML = `
      <section class="fushenglu-section-heading">
        <div><p>與主聊天完全分離 · v${APP_VERSION}</p><h2>API 設定</h2></div>
      </section>
